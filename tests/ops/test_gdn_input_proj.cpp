...
int run_q4_q5() {
    ...
    // c4 SIMT Q5 side at 11..12), 13..31 to the 32x32 tile, T=32 to the 32x32 K=128 grouped tile,
    // and 33..64 to the 32x64 tile, and 65 upward to the 64x128 tile that also supplies the
    // 128-column tail slices. The Q4 K-split band (7..12) is not changed by this work; its two ends
    // are covered by the same list.
    ...
    // slice (129 = 128 + 1).
    for (const std::int32_t tokens : {7, 8, 9, 10, 12, 13, 32, 33, 65, 129}) {
